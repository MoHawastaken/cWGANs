...
class cWGANTrainer:
    ...
    """
    def __init__(self, model, trains, traint = None, datas = None, datat = None,testset = None, gantype = 'wgangp', noise = 'normal'):
    ...
    def get_W1(self, n_wb = 10, types = 'batch', test = False):
        ...
        if(types == 'batches'):
            if(self.traint is None):
                ...
                tbatches = self.generate_images(ys, num_outputs = sbatches.shape[0])
                ...
                if(test):
                    ...
                    faketest = self.generate_images(ytest, num_outputs = testbatches.shape[0])
            ...
            if(self.traint is None):
                tbatch = self.generate_images(ys, num_outputs = sbatch.shape[0])
            gens = torch.cat((tbatch,ys),dim=1)
            reals = torch.cat((sbatches,ys),dim=1)
            if(test):
                testbatch,ytest = self.process_batch(self.itertest)
                faketest = self.generate_images(ytest, num_outputs = sbatch.shape[0])
                gens2 = torch.cat((faketest,ytest),dim=1)
    ...
    def train_estim(self, num_epochs = 1, penalty = 0.1, G_lr=1e-4, D_lr=1e-4, G_wd = 0, D_wd = 0, D_steps_standard=5, n_wb = 10, num_batches = 1, num_estims = 10, onlypot = True):
    ...
    def generate_images(self, y, num_outputs=64):
...
class WGANTrainer:
    """ Object to hold data iterators, train a GAN variant
    """
    def __init__(self, model, trains, traint = None, datas = None, datat = None, testset = None, viz=False, gantype = 'wgangp', noise = 'normal'):
        self.model = to_cuda(model)
        self.name = model.__class__.__name__
        self.noise = noise
        ...
        if(types == 'batches'):
            if(self.traint is None):
                ...
                for i in range(n_wb-1):
                    ...
                    sbatches = torch.cat( (sbatches, sb) ,dim=0)
                tbatches = self.generate_images(num_outputs = sbatches.shape[0])
                if(test):
                    testbatches = self.process_batch(self.itertest)
                    for i in range(n_wb-1):
                        sb = self.process_batch(self.itertest)
                        testbatches = torch.cat( (testbatches, sb) ,dim=0)
                    faketest = self.generate_images(num_outputs = testbatches.shape[0])
            ...
            if(self.traint is None):
                tbatch = self.generate_images(num_outputs = sbatch.shape[0])
            if(test):
                testbatch = self.process_batch(self.itertest)
                faketest = self.generate_images(num_outputs = sbatch.shape[0])
    ...
    def get_W1_old(self, n_wb = 10, types = 'all'):
        if(types == 'all'):
            if(self.datat is None):
                datat = self.generate_images(num_outputs = self.datas.shape[0])
    ...
    def train_D(self, num_epochs = 1, lr = 1e-4, wd = 0, num_batches = 1, num_estims = 10, onlypot = False):
        bet = (0.5, 0.9)
        D_optimizer = optim.Adam(params=[p for p in self.model.D.parameters()
        ...
        for epoch in tqdm(range(1, num_epochs+1)):
            ...
            while(ep_iter < n_batches):
                ...
                if(self.traint is None):
                    other_distr = self.generate_images(num_outputs = distr.shape[0])
                else:
    ...
    def train_estim(self, num_epochs = 1, penalty = 0.1, G_lr=1e-4, D_lr=1e-4, G_wd = 0, D_wd = 0, D_steps_standard=5, n_wb = 10, num_batches = 1, num_estims = 10, onlypot = False):
        """ Train a Wasserstein GAN
            Logs progress using G loss, D loss, G(x), D(G(x)), visualizations
            of Generator output.
        Inputs:
    ...
    def generate_images(self, epoch=-1, num_outputs=64):
        """ Visualize progress of generator learning """
        # Turn off any regularization
        self.model.eval()
